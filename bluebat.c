...
#include <termios.h>    // termios, tcgetattr, tcsetsttr, ECHO, TCSAFLUSH, ICANON, ISIG, IXON, IEXTEN, ICRNL, OPOST, BRKINT, INPCK, ISTRIP, CS8, VMIN, VTIME
#include <time.h>       // time_t, time
#include <unistd.h>     // read, STDIN_FILENO, STDOUT_FILENO, write, ftruncate, close
...
void editorUpdateLineNumberOffSet();
...
int editorRowCxToRx(erow *row, int cx) {
    ...
    for (j = 0; j < cx; j++) {
        if (row->chars[j] == '\t')
            rx += (BLUEBAT_TAB_STOP - 1) - (rx % BLUEBAT_TAB_STOP);
        rx++;
    }
    return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for (cx = 0; cx < row->size; cx++) {
        if (row->chars[cx] == '\t')
            cur_rx += (BLUEBAT_TAB_STOP - 1) - (cur_rx % BLUEBAT_TAB_STOP);
        cur_rx++;
...
void editorUpdateRow(erow *row) {
    ...
    free(row->render);
    row->render = malloc(row->size + tabs * (BLUEBAT_TAB_STOP - 1) + 1);

    int idx = 0;
    for (j = 0; j < row->size; j++) {
        if (row->chars[j] == '\t') {
            row->render[idx++] = ' ';
            while (idx % BLUEBAT_TAB_STOP != 0) row->render[idx++] = ' ';
        } else {
...
void editorSelectChar(int key, int *last_direction) {
    if (*last_direction == 1) {
        if (key == SHIFT_UP && E.ssely == E.esely) {
            E.sselx = E.cx;
            E.ssely = E.cy;
            *last_direction = -1;
        }
            ...
            E.eselx = E.cx;
            E.esely = E.cy;
            if (E.sselx == E.eselx && E.ssely == E.esely) {
                *last_direction = 0;
            }
        }
    }
    else if (*last_direction == -1) {
        if (key == SHIFT_DOWN && E.ssely == E.esely) {
            E.eselx = E.cx;
            E.esely = E.cy;
            *last_direction = 1;
        }
        ...
        if (key == SHIFT_RIGHT || key == SHIFT_DOWN) {
            E.eselx = E.cx;
            E.esely = E.cy;
            *last_direction = 1;
        }
...
void editorInsertNewLine() {
    if (E.cx == 0) {
        ...
        // need to reasign because of realloc on editorInsertRow
        row = &E.row[E.cy];
        row->size = E.cx;
        row->chars[row->size] = '\0';
        editorUpdateRow(row);
...
void editorDrawRows(struct abuf *ab) {
    ...
    for (y = 0; y < E.screenrows; y++) {
        ...
        if (filerow >= E.numrows) {
            if (E.numrows == 0) {
                if (y == E.screenrows / 3) {
                    ...
                    int welcomelen = snprintf(welcome, sizeof(welcome),
                        "Bluebat editor -- version %s", BLUEBAT_VERSION);
            ...
            for (j = 0; j < len; j++) {
                // Highlight selection
                if (y == E.ssely && j == E.sselx) {
                    abAppend(ab, "\x1b[7m", 4);
                }
                else if ((y > E.ssely && y <= E.esely) && j == 0) {
                    abAppend(ab, "\x1b[7m", 4);
                }
                if (y == E.esely && j == E.eselx) {
                    abAppend(ab, "\x1b[m", 3);
                }
...
void editorSelect(int key, int *selecting_direction) {
    ...
    switch (key) {
        ...
        case SHIFT_DOWN:
            ...
            editorSelectChar(SHIFT_DOWN, selecting_direction);
            break;
        case SHIFT_UP:
            editorMoveCursor(ARROW_UP);
            editorSelectChar(SHIFT_UP, selecting_direction);
            break;
    }
}
